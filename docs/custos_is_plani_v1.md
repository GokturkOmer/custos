...
**v1.0.1 mini kalem (pilot öncesi W7 endurance için):**
- **Collector `_tick_miss_count` metriği** — mevcut `slow_tick_ratio` pattern'ine benzer, structlog event olarak yaz. Endurance metrics proxy (journalctl parse) yerine gerçek metriği okusun. ~1 saat iş, pilot öncesi (W7 A5 resmi endurance öncesi, ~25 Mayıs).
...
- **22 Nisan 2026 (8):** **Bulk Import Helper tamamlandı** (planlı ~1 gün, gerçek ~7 saat, aynı gün). 3 atomik commit `7492494..72f12f3` origin/main'de, CI yeşil (3m33s). Backend (`bulk_import.py` — BulkImportRow pydantic whitelist validasyon), 4 route (modal/preview/commit/example indirme — HX-Request'e göre HTML partial ya da JSON), Alpine+HTMX modal UI, 3 örnek dosya (`docs/examples/tag_import_example.csv` + `.yaml` + README). 43 yeni test (26 unit + 8 integration + 9 dashboard), önceki 415 → 458 total pass. **Performans: 200 tag 0.36s parse+validate+DB insert (~20.000x manuel girişe göre).** Saha günü tahmini kazanç 6-7 saat. Duplicate davranışı tüm 3 mod (reject/update/insert) + aynı dosyada duplicate + invalid satır rollback doğrulandı. **Atomik rollback sınırlı:** validation aşaması atomik (tek hata → hiçbiri yazılmaz), runtime DB hatası **kısmi yazıma yol açabilir** çünkü `shared/database.py` transaction context manager expose etmiyor. v1.1 backlog'a eklendi (Bölüm B Akış 3). **TÜM v1.0 KODU BİTTİ — FEATURE-COMPLETE.** Pilot öncesi **~6 hafta erken** tamamlandı (22 Nis → 5 Haz pilot).
- **22 Nisan 2026 (9):** **Endurance Framework tamamlandı** (planlı ~1 gün, gerçek ~6-7 saat, aynı gün — **5. paket 22 Nisan'da**). 6 atomik commit `92e843e..0e71c0a` origin/main'de, CI yeşil. 43 yeni test (12 simülatör + 17 metrics + 14 daily_check), önceki 458 → 492 total pass. **6 script + 1 kılavuz:** simülatör 200 register genişletme (env `CUSTOS_TAG_COUNT=200`, çeşitli dalgalar), `endurance_generate_tags_csv.py` (bulk import CSV üretici), `endurance_metrics.py` (5 dk CSV daemon + SIGTERM + rotate), `endurance_daily_check.py` (günlük rapor + kırmızı bayrak 5 kriter), `endurance_bind_instances.py` (5 instance + role binding), `endurance_setup.sh` (ayrı WSL tek komutluk kurulum), `_personal/pilot/endurance_senaryo_v1.md` (11 bölüm Türkçe kılavuz) + `endurance_tags_200.csv` (200 satır, 150 slow + 45 normal + 5 fast). 11/11 mimari kural yeşil. Smoke test yerel sanity geçti. **3 açık nokta dürüstçe raporlandı:** (a) tick_miss journalctl proxy ile hesaplandı çünkü Collector gerçek telemetry yazmıyor — W7 öncesi `collector.py`'ye `_tick_miss_count` metriği eklenmesi v1.0.1 mini commit kalemi (~1 saat); (b) endurance bind_instances fiziksel birim uyumu yok, amaç trafik üretmek — fabrika pilotu (Temmuz) için binding yeniden planlanacak; (c) `endurance_setup.sh` fresh WSL'de uçtan uca çalıştırılmadı — kullanıcı ilk kez `--skip-wait --skip-training` ile dry-run önerisi takip edecek. **Aynı araç seti 3 kez kullanılacak:** Mayıs başı erken deneme, 28 May - 3 Haz resmi A5, Temmuz fabrika pilotu öncesi. Sıradaki: kullanıcı manuel adımları (WSL kurulum + setup.sh + 7 gün izleme) + kullanım kılavuzu (W6) + A4 subagent review + saha entegrasyonu (arkadaş dönüşü sonrası).
