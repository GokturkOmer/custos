...
from custos.analytics.assistant.service import (
    get_assistant_retriever,
    rebuild_assistant_retriever,
)
...
# --- Knowledge Base Yönetimi (V11-110, hibrit yapı, developer-only) ---

# Slug whitelist'i: küçük harf, rakam, alt çizgi, tire — 1..80 karakter.
# Path traversal koruması: `..`, `/`, `\\` zaten regex dışı kalır. Slug
# kullanıcı input'undan geldiği her noktada bu regex'le doğrulanır.
_KB_SLUG_RE = re.compile(r"^[a-z0-9_-]{1,80}$")

# Lokal doküman içerik üst sınırı (byte). Pilot KB dokümanları tipik olarak
# 1-5 KB; 100 KB form post'una serbest tavan, DoS koruması.
_KB_MAX_BODY_BYTES = 100 * 1024


def _kb_local_dir() -> Path:
    """Lokal (saha-spesifik) KB dizini — config'ten."""
    return Path(settings.custos_assistant_knowledge_local_dir)


def _kb_validate_slug(slug: str) -> str:
    """Slug whitelist + path traversal koruması.

    Geçerli slug'ı geri döndürür; geçersizse 400 fırlatır. URL'den gelen
    {slug} parametresi ve form'dan gelen slug input'u aynı kontrolden
    geçer.
    """
    if not _KB_SLUG_RE.match(slug):
        raise HTTPException(
            status_code=400,
            detail=(
                "Geçersiz slug — küçük harf, rakam, _ ve - "
                "(1-80 karakter)"
            ),
        )
    return slug
...
def _kb_local_md_path(slug: str) -> Path:
    """Validated slug için lokal `.md` yolu. Lokal dizin yoksa kurar
    (dev makinesinde de çalışsın diye; setup.sh prod'da kurar)."""
    target_dir = _kb_local_dir()
    target_dir.mkdir(parents=True, exist_ok=True)
    return target_dir / f"{_kb_validate_slug(slug)}.md"
...
async def knowledge_page(request: Request) -> HTMLResponse:
    """KB yönetim sayfası — git docs (read-only) + lokal docs (CRUD).

    Operator erişemez (require_developer). Liste loader'ın
    `summarize_documents` helper'ı ile üretilir; her dokümanın slug,
    title, kategori, format bilgisi gösterilir.
    """
    git_dir = Path(settings.custos_assistant_knowledge_dir)
    local_dir = _kb_local_dir()

    git_docs = summarize_documents(git_dir)
    local_docs = summarize_documents(local_dir)

    # Override görünümü: lokal'de aynı slug varsa git tarafında "override
    # ediliyor" rozeti gösterilir (kullanıcı bilsin ki bu git dokümanı
    # asistan tarafından görünmüyor; lokal kazandı).
    local_slugs = {d.slug for d in local_docs}
    git_view = [
        {
            "slug": d.slug,
            "title": d.title,
            ...
            "overridden_by_local": d.slug in local_slugs,
        }
        for d in git_docs
    ]
    local_view = [
    ...
    ctx = _base_context(
        ...
        active_nav="/dashboard/settings",
        git_docs=git_view,
        local_docs=local_view,
        git_dir=str(git_dir),
        local_dir=str(local_dir),
        total_count=len(git_docs) + len(local_docs),
        error=request.query_params.get("error"),
        success=request.query_params.get("ok"),
    )
    return templates.TemplateResponse(request, "pages/knowledge.html", ctx)
...
async def knowledge_local_new_form(request: Request) -> HTMLResponse:
    """Yeni lokal doküman formu — boş slug + boş textarea."""
    ctx = _base_context(
        page_title="Yeni Lokal Doküman",
        active_nav="/dashboard/settings",
        mode="new",
        slug="",
        body="",
    ...
    return templates.TemplateResponse(
        request, "pages/knowledge_local_form.html", ctx
    )
...
async def knowledge_local_create(
    request: Request,
    slug: str = Form(...),
    body: str = Form(...),
) -> RedirectResponse:
    """Yeni lokal doküman kaydeder. Aynı slug varsa hata."""
    slug_norm = slug.strip().lower()
    try:
        target = _kb_local_md_path(slug_norm)
    except HTTPException:
        return RedirectResponse(
            url="/dashboard/knowledge/local/new?error=invalid_slug",
            status_code=303,
        )
    if target.exists():
        return RedirectResponse(
            url="/dashboard/knowledge/local/new?error=duplicate",
            status_code=303,
        )
    body_bytes = body.encode("utf-8")
    if len(body_bytes) > _KB_MAX_BODY_BYTES:
        return RedirectResponse(
            url="/dashboard/knowledge/local/new?error=too_large",
            status_code=303,
        )
    ...
    await db.insert_audit_log(
        AuditLogEntry(
            category="knowledge",
            action="local_doc_created",
            entity_type="kb_doc",
            entity_id=slug_norm,
            detail=f"{len(body_bytes)} bayt",
        )
    )
    return RedirectResponse(url="/dashboard/knowledge?ok=created", status_code=303)


@router.get(
    "/knowledge/local/edit/{slug}",
    response_class=HTMLResponse,
    dependencies=[_DEVELOPER_DEP],
)
async def knowledge_local_edit_form(
    request: Request,
    slug: str,
) -> HTMLResponse:
    """Lokal doküman düzenleme formu — mevcut içerik textarea'ya yüklenir."""
    target = _kb_local_md_path(slug)
    if not target.exists():
        raise HTTPException(status_code=404, detail="Lokal doküman bulunamadı")
    body = target.read_text(encoding="utf-8")
    ctx = _base_context(
        page_title=f"Düzenle: {slug}",
        active_nav="/dashboard/settings",
        mode="edit",
        slug=slug,
        body=body,
...
async def knowledge_local_update(
    request: Request,
    slug: str,
    body: str = Form(...),
) -> RedirectResponse:
    """Lokal doküman içeriğini günceller (slug değişmez)."""
    target = _kb_local_md_path(slug)
    if not target.exists():
        raise HTTPException(status_code=404, detail="Lokal doküman bulunamadı")
    body_bytes = body.encode("utf-8")
    if len(body_bytes) > _KB_MAX_BODY_BYTES:
        return RedirectResponse(
            url=f"/dashboard/knowledge/local/edit/{slug}?error=too_large",
            status_code=303,
        )
    target.write_text(body, encoding="utf-8")
    ...
    await db.insert_audit_log(
        AuditLogEntry(
            category="knowledge",
            action="local_doc_updated",
            entity_type="kb_doc",
            entity_id=slug,
            detail=f"{len(body_bytes)} bayt",
        )
    )
    return RedirectResponse(url="/dashboard/knowledge?ok=updated", status_code=303)


@router.post(
    "/knowledge/local/delete/{slug}",
    response_class=HTMLResponse,
    dependencies=[_DEVELOPER_DEP],
)
async def knowledge_local_delete(
    request: Request,
    slug: str,
) -> RedirectResponse:
    """Lokal dokümanı siler."""
    target = _kb_local_md_path(slug)
    if not target.exists():
        raise HTTPException(status_code=404, detail="Lokal doküman bulunamadı")
    target.unlink()
    ...
    await db.insert_audit_log(
        AuditLogEntry(
            category="knowledge",
            action="local_doc_deleted",
            entity_type="kb_doc",
            entity_id=slug,
        )
    )
    return RedirectResponse(url="/dashboard/knowledge?ok=deleted", status_code=303)
...
async def knowledge_rebuild_index(request: Request) -> RedirectResponse:
    """FAISS indeksini sync olarak yeniden kurar.

    `rebuild_assistant_retriever()` ~2-3 sn sürer (24 doküman, embedding).
    Background thread yerine sync — pilot ölçeğinde race-free + basit.
    UI tarafında HTMX `hx-indicator` ile loading state gösterilir.
    """
    await asyncio.to_thread(rebuild_assistant_retriever)
