...
async def _seed_readings(
    ...
    start: datetime, count: int, step_seconds: float = 1.0,
) -> None:
    ...
    for i in range(count):
        ...
        batch.append(TagReading(
            timestamp=ts, tag_id=tag_id,
...
async def test_downsample_reduces_to_target_points(
    ...
    await db.insert_tag(TagRecord(
        tag_id=tag_id, name="Downsample Test",
    ...
    result = await db.query_tag_readings_downsampled(
        tag_id, start, end, target_points=100,
    )
...
async def test_downsample_bucket_value_is_average(
    ...
    await db.insert_tag(TagRecord(
        tag_id=tag_id, name="Avg Test",
        modbus_host="127.0.0.1", register_address=40002,
    ))
    ...
    batch = [
        TagReading(
            ...
            tag_id=tag_id, value=5.0, quality_flag=0,
        )
        for i in range(30)
    ]
    await db.insert_tag_readings_batch(batch)
    ...
    result = await db.query_tag_readings_downsampled(
        tag_id, start, end, target_points=10,
    )
    assert len(result) >= 1
...
async def test_downsample_empty_range_returns_empty(
    ...
    await db.insert_tag(TagRecord(
        tag_id=tag_id, name="Empty Test",
        modbus_host="127.0.0.1", register_address=40003,
    ))
...
async def test_downsample_preserves_order(
    ...
    await db.insert_tag(TagRecord(
        tag_id=tag_id, name="Order Test",
        modbus_host="127.0.0.1", register_address=40004,
    ))
    ...
    result = await db.query_tag_readings_downsampled(
        tag_id, start, end, target_points=20,
    )
    assert len(result) >= 2
