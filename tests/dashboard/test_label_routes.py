...
class _LabelMockDB:
    ...
    def _attach_label(self, event: AlarmEvent) -> AlarmEvent:
        """R-05a: SELECT'lerin LEFT JOIN davranışını taklit et."""
        if event.id is None:
            return event
        return replace(event, label=self._labels.get(event.id))

    async def get_alarm_event(self, event_id: int) -> AlarmEvent | None:
        event = self._alarms.get(event_id)
        if event is None:
            return None
        return self._attach_label(event)
    ...
    async def list_alarm_events(
        ...
        # R-05a: LEFT JOIN sonucu — her alarm label'ı ile birlikte döner.
        results = [
            self._attach_label(a)
